...
import com.game.model.GameUser;
...
public class ResetPassPanel extends JPanel {
    ...
    public ResetPassPanel(AccessFrame frame) {
        ...
        JLabel titleLabel = createGamingLabel("RESET PASSWORD", 36);
        ...
        JPanel usernamePanel = createGamingInputPanel("ENTER NEW PASSWORD", passwordField = createGamingTextField());
        
        // Password field with gaming style
        JPanel passwordPanel = createGamingInputPanel("RE-ENTER NEW PASSWORD", repasswordField = createGamingTextField());
        
        // Login button with gaming style
        loginButton = createGamingButton("SIGN UP");
        loginButton.addActionListener(e -> resetpass());

        JPanel registerPanel = new JPanel();
        registerPanel.setOpaque(false);
        registerPanel.setLayout(new FlowLayout(FlowLayout.CENTER));

        JLabel registerLabel = new JLabel("Dùng tài khoản khác? ");
    ...
    private JPasswordField createGamingTextField() {
    ...
    private void resetpass() {
        ...
        if (pass.equals(repass)) {
            try {
                ...
                requestBody.put("id", user.getId().toString());
                requestBody.put("username", user.getUsername());
                requestBody.put("password", pass);
                requestBody.put("email", user.getEmail());
                requestBody.put("tiendo", String.valueOf(user.getTiendo()));
                requestBody.put("trangthai", "Đã kích hoạt");
